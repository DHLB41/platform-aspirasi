...
export class CreateVolunteerTables1757407722000 implements MigrationInterface {
    name = 'CreateVolunteerTables1757407722000';
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        // Create ENUMs first
        await queryRunner.query(`
            CREATE TYPE media_type_enum AS ENUM ('image', 'document', 'video', 'audio')
        `);
        
        await queryRunner.query(`
            CREATE TYPE media_status_enum AS ENUM ('uploaded', 'processing', 'ready', 'failed')
        `);
        ...
        await queryRunner.query(`
            CREATE TYPE family_member_status_enum AS ENUM ('active', 'deceased', 'separated')
        `);
        ...
        await queryRunner.query(`
            CREATE TYPE work_area_type_enum AS ENUM ('village', 'district', 'city', 'province', 'custom')
        `);

        await queryRunner.query(`
            CREATE TYPE work_area_status_enum AS ENUM ('active', 'inactive', 'planned')
        `);

        await queryRunner.query(`
            CREATE TYPE volunteer_status_enum AS ENUM ('pending', 'active', 'inactive', 'suspended')
        `);

        // Add missing marital_status_enum that's used in volunteers table
        await queryRunner.query(`
        ...
        // Create volunteers table - Fixed to match entity structure
        await queryRunner.query(`
        ...
        await queryRunner.query(`CREATE INDEX "IDX_volunteers_user_id" ON "volunteers" ("user_id")`);
        await queryRunner.query(`CREATE INDEX "IDX_volunteers_work_area_id" ON "volunteers" ("work_area_id")`);
        await queryRunner.query(`CREATE INDEX "IDX_volunteers_status" ON "volunteers" ("status")`);
        await queryRunner.query(`CREATE INDEX "IDX_volunteers_nik" ON "volunteers" ("nik")`);
        ...
        await queryRunner.query(`CREATE INDEX "IDX_family_members_status" ON "family_members" ("status")`);
        ...
        const userTriggerExists = await queryRunner.query(`
            SELECT trigger_name FROM information_schema.triggers 
            WHERE event_object_table = 'users' 
            AND trigger_name = 'update_users_updated_at'
        `);

        if (!userTriggerExists || userTriggerExists.length === 0) {
            await queryRunner.query(`
        ...
        // Add trigger for refresh_tokens table if it doesn't exist
        const tokenTriggerExists = await queryRunner.query(`
            SELECT trigger_name FROM information_schema.triggers 
            WHERE event_object_table = 'refresh_tokens' 
            AND trigger_name = 'update_refresh_tokens_updated_at'
        `);

        if (!tokenTriggerExists || tokenTriggerExists.length === 0) {
            await queryRunner.query(`
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`DROP INDEX IF EXISTS "IDX_family_members_status"`);
        ...
        await queryRunner.query(`DROP INDEX IF EXISTS "IDX_volunteers_work_area_id"`);
